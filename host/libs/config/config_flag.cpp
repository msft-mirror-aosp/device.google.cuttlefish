...
bool ShouldPadWithUnset(const std::string& flag) {
  ...
  // Flags starting with "display" (e.g. display, display0, display1) use internal
  // comma-separated key=value pairs (width=...,height=...) and cannot be comma-vectorized.
  // Flags starting with "webrtc_" or "ap_" are also non-vectorized.
  if (android::base::StartsWith(flag, "display") ||
      android::base::StartsWith(flag, "webrtc_") ||
      android::base::StartsWith(flag, "ap_")) {
    return false;
  }
  return true;
}
