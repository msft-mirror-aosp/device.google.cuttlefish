...
std::set<pid_t> GetCandidateProcessGroups() {
  ...
  auto instance_dir = cuttlefish::StringFromEnv("HOME", ".") + "/cuttlefish_runtime";
  cmd += " " + instance_dir;
  ...
  cmd += ((" " + instance_dir + "/") + cuttlefish::kInternalDirName) + "/*";
  // Add the shared memory file
  cmd += " " + cuttlefish::GetPerInstanceDefault("/dev/shm/cvd-");
