...
Result<std::vector<GuestConfig>> ReadGuestConfig() {
  ...
  for (int instance_index = 0; instance_index < instance_nums.size(); instance_index++) {
    ...
    // in case 'instance_index >= system_image_dir.size()' it is the same image
    // being launhced multiple times the same flag is used for all instances
    const auto& system_dir =
        system_image_dir[instance_index >= system_image_dir.size()
                             ? 0
                             : instance_index];

    constexpr char kGuestConfigFilename[] = "cuttlefish-guest-config.txtpb";
    const std::string guest_config_path =
        system_dir + "/" + kGuestConfigFilename;
    if (FileExists(guest_config_path)) {
      LOG(INFO) << "Guest config found: " << guest_config_path;
      CF_EXPECT(ParseGuestConfigTextProto(guest_config_path, guest_config),
                "Failed to parse textproto.");
    } else {
      const std::string android_info_txt_path =
          system_dir + "/android-info.txt";
      LOG(INFO) << "Defaulting to `android-info.txt` because guest protobuf "
                   "config is missing.";
