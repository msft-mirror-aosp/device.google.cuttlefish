...
Result<std::unordered_map<std::string, std::string>> BootconfigArgsFromConfig(
  ...
  // Export modem simulator ports for both standalone modem_simulator and netsim.
  // The guest RIL (ag/40529282) polls ro.boot.modem_simulator_ports on startup to
  // connect over VSOCK regardless of which daemon handles the host connection.
