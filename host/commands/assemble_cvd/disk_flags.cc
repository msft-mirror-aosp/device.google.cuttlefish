...
bool ResolveInstanceFiles() {
  ...
  std::string default_vbmeta_system_image =
      FLAGS_system_image_dir + "/vbmeta_system.img";
...
static bool DecompressKernel(const std::string &src, const std::string &dst) {
  ...
  decomp_cmd.RedirectStdIO(cuttlefish::Subprocess::StdIOChannel::kStdOut,
                           output_file);
...
static bool ConcatRamdisks(const std::string &new_ramdisk_path,
  ...
  std::ofstream new_ramdisk(new_ramdisk_path,
...
static off_t AvailableSpaceAtPath(const std::string &path) {
  ...
  return vfs.f_bsize *
         vfs.f_bavail; // block size * free blocks for unprivileged users
}

bool CreateCompositeDisk(const cuttlefish::CuttlefishConfig &config) {
  if (!cuttlefish::SharedFD::Open(config.composite_disk_path().c_str(),
                                  O_WRONLY | O_CREAT, 0644)
           ->IsOpen()) {
    LOG(ERROR) << "Could not ensure " << config.composite_disk_path()
               << " exists";
    return false;
  }
  if (config.vm_manager()== CrosvmManager::name()) {
    ...
    if (available_space <
      ...
      // separate machine
      LOG(ERROR) << "Not enough space remaining in fs containing "
                 << FLAGS_data_image;
      LOG(ERROR) << "Wanted "
                 << (existing_sizes.sparse_size - existing_sizes.disk_size);
...
void CreateDynamicDiskFiles(const cuttlefish::FetcherConfig &fetcher_config,
  ...
  if (!boot_img_unpacker->Unpack(
          config->ramdisk_image_path(), config->vendor_ramdisk_image_path(),
  ...
  std::string foreign_ramdisk =
      FLAGS_initramfs_path.size() ? FLAGS_initramfs_path : discovered_ramdisk;
  ...
  for (const auto &instance : config->Instances()) {
    ...
    if (!cuttlefish::FileExists(instance.sdcard_path())) {
      CreateBlankImage(instance.sdcard_path(), FLAGS_blank_sdcard_image_mb,
                       "sdcard");
    }
