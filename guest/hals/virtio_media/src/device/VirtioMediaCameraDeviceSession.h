...
class VirtioMediaCameraDeviceSession : public BnCameraDeviceSession,
 ...
 public:
  ...
  ScopedAStatus configureStreams(
  ...
  Status processCaptureRequestError(
  ...
  class OutputThread : public SimpleThread {
   public:
    ...
    Status allocateIntermediateBuffers(const Size& v4lSize,
   ...
   protected:
    ...
    int cropAndScaleLocked(std::shared_ptr<AllocatedFrame>& in,
                           const Size& outSize, YCbCrLayout* out);

    int cropAndScaleThumbLocked(std::shared_ptr<AllocatedFrame>& in,
                                const Size& outSize, YCbCrLayout* out);
    ...
    std::unordered_map<Size, std::shared_ptr<AllocatedFrame>, SizeHasher>
 ...
 private:
  ...
  int configureV4l2StreamLocked(const SupportedV4L2Format& fmt,
                                double fps = 0.0);
  Status selectV4L2Format(const StreamConfiguration& config,
  ...
  Status importBufferLocked(int32_t streamId, uint64_t bufId,
                            buffer_handle_t buf,
