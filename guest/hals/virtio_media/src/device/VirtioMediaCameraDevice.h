...
class VirtioMediaCameraDevice : public BnCameraDevice {
 public:
  VirtioMediaCameraDevice(const std::string& devicePath,
                          const ExternalCameraConfig& config);
  ~VirtioMediaCameraDevice() override;
 ...
 private:
  ...
  status_t initOutputCharsKeysByFormat(
      ...
      int streamConfigurationKey, int minFrameDurationKey,
      int stallDurationKey);

  status_t calculateMinFps(
      ::android::hardware::camera::common::V1_0::helper::CameraMetadata*);

  static void getFrameRateList(int fd, double fpsUpperBound,
                               SupportedV4L2Format* format);

  static void updateFpsBounds(
