...
// Size of request/result metadata fast message queue. Change to 0 to always use
// hwbinder buffer.
static constexpr size_t kMetadataMsgQueueSize = 1 << 18 /* 256kB */;

const int kBadFramesAfterStreamOn =
    1;  // drop x frames after streamOn to get rid of some initial
        // bad frames. TODO: develop a better bad frame detection
        // method
constexpr int MAX_RETRY =
    15;  // Allow retry some ioctl failures a few times to account for some
...
// Static instances
const int VirtioMediaCameraDeviceSession::kMaxProcessedStream;
const int VirtioMediaCameraDeviceSession::kMaxStallStream;
HandleImporter VirtioMediaCameraDeviceSession::sHandleImporter;
...
Size VirtioMediaCameraDeviceSession::getMaxThumbResolution() const {
  return getMaxThumbnailResolution(mCameraCharacteristics);
}

Size VirtioMediaCameraDeviceSession::getMaxJpegResolution() const {
  Size ret{0, 0};
...
void VirtioMediaCameraDeviceSession::initOutputThread() {
  // Grab a shared_ptr to 'this' from ndk::SharedRefBase::ref()
  std::shared_ptr<VirtioMediaCameraDeviceSession> thiz =
      ref<VirtioMediaCameraDeviceSession>();

  mBufferRequestThread =
      std::make_shared<BufferRequestThread>(/*parent=*/thiz, mCallback);
...
void VirtioMediaCameraDeviceSession::closeOutputThread() {
  if (mOutputThread != nullptr) {
    mOutputThread->flush();
...
void VirtioMediaCameraDeviceSession::closeBufferRequestThread() {
...
Status VirtioMediaCameraDeviceSession::initStatus() const {
  Mutex::Autolock _l(mLock);
  Status status = Status::OK;
  if (mInitFail || mClosed) {
    ALOGI("%s: session initFailed %d closed %d", __FUNCTION__, mInitFail,
          mClosed);
...
VirtioMediaCameraDeviceSession::~VirtioMediaCameraDeviceSession() {
  if (!isClosed()) {
    ALOGE("VirtioMediaCameraDeviceSession deleted before close!");
    closeImpl();
  }
}
...
ScopedAStatus VirtioMediaCameraDeviceSession::constructDefaultRequestSettings(
  ...
  switch (type) {
    ...
    case RequestTemplate::VIDEO_SNAPSHOT: {
      *_aidl_return = mDefaultRequests[type];
      break;
    }
    case RequestTemplate::ZERO_SHUTTER_LAG:
      // Don't support ZSL templates
...
ScopedAStatus VirtioMediaCameraDeviceSession::configureStreams(
  ...
  Status status = isStreamCombinationSupported(in_requestedConfiguration,
...

  {
    ...
    if (!mInflightFrames.empty()) {
      ALOGE(
          "%s: trying to configureStreams while there are still %zu inflight "
          "frames!",
  ...
  status = selectV4L2Format(in_requestedConfiguration, &v4l2Fmt);
  if (status != Status::OK) {
    return fromStatus(status);
  }
  ...
  if (configureV4l2StreamLocked(v4l2Fmt) != 0) {
    ALOGE("V4L configuration failed!, format:%c%c%c%c, w %d, h %d",
          v4l2Fmt.fourcc & 0xFF, (v4l2Fmt.fourcc >> 8) & 0xFF,
          (v4l2Fmt.fourcc >> 16) & 0xFF, (v4l2Fmt.fourcc >> 24) & 0xFF,
  ...
  status = mOutputThread->allocateIntermediateBuffers(
      v4lSize, mMaxThumbResolution, in_requestedConfiguration.streams,
      blobBufferSize);
  if (status != Status::OK) {
  ...
  for (size_t i = 0; i < in_requestedConfiguration.streams.size(); i++) {
    ...
    mStreamMap[in_requestedConfiguration.streams[i].id].usage =
        out[i].producerUsage = static_cast<BufferUsage>(
            ((int64_t)in_requestedConfiguration.streams[i].usage) |
    ...
    switch (in_requestedConfiguration.streams[i].format) {
      ...
      case PixelFormat::IMPLEMENTATION_DEFINED:
        ...
        // This should look at the Stream's dataspace flag to determine the
        // format or leave it as is if the rest of the system knows how to
        // handle a private format. To keep this HAL generic, this is being
        // overridden to YUV420
        out[i].overrideFormat = PixelFormat::YCBCR_420_888;
        // Save overridden format in mStreamMap
        mStreamMap[in_requestedConfiguration.streams[i].id].format =
            out[i].overrideFormat;
        break;
      default:
...
Status VirtioMediaCameraDeviceSession::processOneCaptureRequest(
  ...
  for (auto& outputBuf : request.outputBuffers) {
    if (outputBuf.streamId == -1 ||
        mStreamMap.find(outputBuf.streamId) == mStreamMap.end()) {
      ALOGE("%s: Invalid streamId in CaptureRequest.outputBuffers: %d",
            __FUNCTION__, outputBuf.streamId);
      return Status::ILLEGAL_ARGUMENT;
    }
  }
  ...
  if (request.fmqSettingsSize > 0) {
    ...
    bool read = mRequestMetadataQueue->read(
        reinterpret_cast<int8_t*>(settingsFmq.metadata.data()),
  ...
  if (numOutputBufs == 0) {
    ALOGE("%s: capture request must have at least one output buffer!",
          __FUNCTION__);
    return Status::ILLEGAL_ARGUMENT;
  }

  camera_metadata_entry fpsRange =
      mLatestReqSetting.find(ANDROID_CONTROL_AE_TARGET_FPS_RANGE);
  if (fpsRange.count == 2) {
    ...
    if (!fpsSupported) {
       ...
       * 1. The application is sending an FPS range not supported by the
       * configured outputs.
       * 2. The application is sending a valid FPS range for all configured
       * outputs, but the selected V4L2 size can only run at slower speed. This
       * should be very rare though: for this to happen a sensor needs to
       * support at least 3 different aspect ratio outputs, and when (at least)
       * two outputs are both not the main aspect ratio of the webcam, a third
       * size that's larger might be picked and runs into this issue.
       */
      ALOGW("%s: cannot reach fps %d! Will do %f instead", __FUNCTION__,
            fpsRange.data.i32[1], closestFps);
  ...
  for (size_t i = 0; i < numOutputBufs; i++) {
    ...
    halBuf.bufPtr =
        nullptr;  // threadloop will request buffer from cameraservice
    halBuf.acquireFence =
        0;  // threadloop will request fence from cameraservice
...
ScopedAStatus VirtioMediaCameraDeviceSession::signalStreamFlush(
    ...
    int32_t in_streamConfigCounter) {
  {
    ...
    if (in_streamConfigCounter < mLastStreamConfigCounter) {
      // stale call. new streams have been configured since this call was
      // issued. Do nothing.
      return fromStatus(Status::OK);
    }
  }
...
ScopedAStatus VirtioMediaCameraDeviceSession::switchToOffline(
  ...
  Status st =
      switchToOffline(in_streamsToKeep, &msgs, &results, &info, &session);
...
Status VirtioMediaCameraDeviceSession::switchToOffline(
  ...
  if (msgs == nullptr || results == nullptr || info == nullptr ||
      session == nullptr) {
    ALOGE("%s, output arguments (%p, %p, %p, %p) must not be null",
          __FUNCTION__, msgs, results, info, session);
  }
      ...
      if (dropBuffer) {
        ...
        if (buffer.acquireFence >= 0) {
          native_handle_t* handle =
              native_handle_create(/*numFds*/ 1, /*numInts*/ 0);
  ...
  for (auto& v4lReq : halReqs) {
    ...
    std::shared_ptr<V4L2Frame> v4l2Frame(
        static_cast<V4L2Frame*>(v4lReq->frameIn.get()));
  ...
  std::shared_ptr<ExternalCameraOfflineSession> sessionImpl =
      ndk::SharedRefBase::make<ExternalCameraOfflineSession>(
          mCroppingType, mCameraCharacteristics, mCameraId, mExifMake,
          mExifModel, mBlobBufferSize, afTrigger, streamInfos, offlineReqs,
          circulatingBuffers);
...
status_t VirtioMediaCameraDeviceSession::initDefaultRequests() {
  ...
  UPDATE(md, ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION, &exposureCompensation,
         1);

  const uint8_t videoStabilizationMode =
      ANDROID_CONTROL_VIDEO_STABILIZATION_MODE_OFF;
  UPDATE(md, ANDROID_CONTROL_VIDEO_STABILIZATION_MODE, &videoStabilizationMode,
         1);
  ...
  UPDATE(md, ANDROID_CONTROL_AE_TARGET_FPS_RANGE, defaultFpsRange,
         ARRAY_SIZE(defaultFpsRange));
...
status_t VirtioMediaCameraDeviceSession::fillCaptureResult(
...
int VirtioMediaCameraDeviceSession::configureV4l2StreamLocked(
  ...
  if (isMultiPlanar(mCaptureType)) {
    fmt.fmt.pix_mp.width = v4l2Fmt.width;
    fmt.fmt.pix_mp.height = v4l2Fmt.height;
    fmt.fmt.pix_mp.pixelformat = v4l2Fmt.fourcc;
    fmt.fmt.pix_mp.num_planes = getPlaneCountForFormat(v4l2Fmt.fourcc);
  } else {
  ...
  if (mCaptureType != fmt.type) {
    ALOGE("%s: S_FMT returned mismatched type %d, expected %d", __FUNCTION__,
          fmt.type, mCaptureType);
    return -EINVAL;
  }

  if (isMultiPlanar(mCaptureType)) {
    if (v4l2Fmt.width != fmt.fmt.pix_mp.width ||
        v4l2Fmt.height != fmt.fmt.pix_mp.height ||
        v4l2Fmt.fourcc != fmt.fmt.pix_mp.pixelformat) {
      ALOGE("%s: S_FMT expect %c%c%c%c %dx%d, got %c%c%c%c %dx%d instead!",
            __FUNCTION__, v4l2Fmt.fourcc & 0xFF, (v4l2Fmt.fourcc >> 8) & 0xFF,
            (v4l2Fmt.fourcc >> 16) & 0xFF, (v4l2Fmt.fourcc >> 24) & 0xFF,
            v4l2Fmt.width, v4l2Fmt.height, fmt.fmt.pix_mp.pixelformat & 0xFF,
            (fmt.fmt.pix_mp.pixelformat >> 8) & 0xFF,
            (fmt.fmt.pix_mp.pixelformat >> 16) & 0xFF,
            (fmt.fmt.pix_mp.pixelformat >> 24) & 0xFF, fmt.fmt.pix_mp.width,
            fmt.fmt.pix_mp.height);
      return -EINVAL;
    }
  } else {
    if (v4l2Fmt.width != fmt.fmt.pix.width ||
        v4l2Fmt.height != fmt.fmt.pix.height ||
        v4l2Fmt.fourcc != fmt.fmt.pix.pixelformat) {
      ALOGE("%s: S_FMT expect %c%c%c%c %dx%d, got %c%c%c%c %dx%d instead!",
            __FUNCTION__, v4l2Fmt.fourcc & 0xFF, (v4l2Fmt.fourcc >> 8) & 0xFF,
            (v4l2Fmt.fourcc >> 16) & 0xFF, (v4l2Fmt.fourcc >> 24) & 0xFF,
            v4l2Fmt.width, v4l2Fmt.height, fmt.fmt.pix.pixelformat & 0xFF,
            (fmt.fmt.pix.pixelformat >> 8) & 0xFF,
            (fmt.fmt.pix.pixelformat >> 16) & 0xFF,
            (fmt.fmt.pix.pixelformat >> 24) & 0xFF, fmt.fmt.pix.width,
            fmt.fmt.pix.height);
      return -EINVAL;
    }
  }
  ...
  if (isMultiPlanar(mCaptureType)) {
    mV4L2PlaneCount = fmt.fmt.pix_mp.num_planes;
    mV4L2BytesPerLine.resize(mV4L2PlaneCount);
    for (uint32_t i = 0; i < mV4L2PlaneCount; i++) {
      mV4L2BytesPerLine[i] = fmt.fmt.pix_mp.plane_fmt[i].bytesperline;
      bufferSize += fmt.fmt.pix_mp.plane_fmt[i].sizeimage;
    }
  } else {
    mV4L2PlaneCount = 1;
    mV4L2BytesPerLine.resize(1);
    mV4L2BytesPerLine[0] = fmt.fmt.pix.bytesperline;
    bufferSize = fmt.fmt.pix.sizeimage;
  }
  ALOGI("%s: V4L2 buffer size is %d", __FUNCTION__, bufferSize);

  uint32_t expectedMaxBufferSize = 0;
  if (isMultiPlanar(mCaptureType)) {
    expectedMaxBufferSize =
        kMaxBytesPerPixel * fmt.fmt.pix_mp.width * fmt.fmt.pix_mp.height;
  } else {
    expectedMaxBufferSize =
  ...
  if (bufferSize == 0) {
    ...
    ALOGW(
        "%s: V4L2 buffer size: %u, larger than maximum size: %u, clamping to "
        "%u",
  ...
  if (requestFps != 0.0) {
    ...
    // No fps > 30 found, use the highest fps available within supported
    // formats.
  ...
  uint32_t v4lBufferCount =
      (fps >= kDefaultFps) ? mCfg.numVideoBuffers : mCfg.numStillBuffers;

  // Double the max lag in theory.
  mMaxLagNs = v4lBufferCount * 1000000000LL * 2 / fps;
  ALOGI("%s: set mMaxLagNs to %" PRIu64 " ns, v4lBufferCount %u", __FUNCTION__,
        mMaxLagNs, v4lBufferCount);
  ...
  if (TEMP_FAILURE_RETRY(ioctl(mV4l2Fd.get(), VIDIOC_REQBUFS, &req_buffers)) <
      0) {
  ...
  mV4L2Buffers.clear();
  mV4L2Buffers.resize(mV4L2BufferCount);

  for (uint32_t i = 0; i < mV4L2BufferCount; i++) {
    V4L2BufferInfo& info = mV4L2Buffers[i];
    memset(&info.buffer, 0, sizeof(v4l2_buffer));
    info.buffer.index = i;
    info.buffer.type = mCaptureType;
    info.buffer.memory = V4L2_MEMORY_MMAP;

    if (isMultiPlanar(mCaptureType)) {
      memset(info.planes, 0, sizeof(info.planes));
      info.buffer.m.planes = info.planes;
      info.buffer.length = VIDEO_MAX_PLANES;
    }

    if (TEMP_FAILURE_RETRY(
            ioctl(mV4l2Fd.get(), VIDIOC_QUERYBUF, &info.buffer)) < 0) {
      ALOGE("%s: QUERYBUF %d failed: %s", __FUNCTION__, i, strerror(errno));
      return -errno;
    }

    if (isMultiPlanar(mCaptureType)) {
      for (uint32_t j = 0; j < info.buffer.length; j++) {
        void* addr =
            mmap(nullptr, info.planes[j].length, PROT_READ | PROT_WRITE,
                 MAP_SHARED, mV4l2Fd.get(), info.planes[j].m.mem_offset);
        if (addr == MAP_FAILED) {
          ALOGE("%s: mmap buffer %d plane %d failed: %s", __FUNCTION__, i, j,
                strerror(errno));
          v4l2StreamOffLocked();
          return -errno;
        }
        info.mMappedAllocations.emplace_back(addr, info.planes[j].length);
        info.planeAddrs.push_back(addr);
        info.planeSizes.push_back(info.planes[j].length);
      }
    } else {
      void* addr = mmap(nullptr, info.buffer.length, PROT_READ | PROT_WRITE,
                        MAP_SHARED, mV4l2Fd.get(), info.buffer.m.offset);
      if (addr == MAP_FAILED) {
        ALOGE("%s: mmap buffer %d failed: %s", __FUNCTION__, i,
              strerror(errno));
        v4l2StreamOffLocked();
        return -errno;
      }
      info.mMappedAllocations.emplace_back(addr, info.buffer.length);
      info.planeAddrs.push_back(addr);
      info.planeSizes.push_back(info.buffer.length);
    }

    if (isMultiPlanar(mCaptureType)) {
      info.buffer.length = mV4L2PlaneCount;
    }

    if (TEMP_FAILURE_RETRY(ioctl(mV4l2Fd.get(), VIDIOC_QBUF, &info.buffer)) <
        0) {
...

  {
    ...
    v4l2_buf_type capture_type = mCaptureType;
    int numAttempt = 0;
    do {
      ret = TEMP_FAILURE_RETRY(
          ioctl(mV4l2Fd.get(), VIDIOC_STREAMON, &capture_type));
      if (numAttempt == MAX_RETRY) {
        break;
      }
      numAttempt++;
      if (ret < 0) {
        ALOGW("%s: VIDIOC_STREAMON failed, wait 33ms and try again",
              __FUNCTION__);
    ...
    if (ret < 0) {
      ALOGE("%s: VIDIOC_STREAMON ioctl failed: %s", __FUNCTION__,
            strerror(errno));
      v4l2StreamOffLocked();
      return -errno;
    }
  ...
  // Swallow first few frames after streamOn to account for bad frames from some
  // devices
  for (int i = 0; i < kBadFramesAfterStreamOn; i++) {
    ...
    if (isMultiPlanar(mCaptureType)) {
      buffer.length = mV4L2PlaneCount;
    }

    if (TEMP_FAILURE_RETRY(ioctl(mV4l2Fd.get(), VIDIOC_QBUF, &buffer)) < 0) {
      ALOGE("%s: QBUF index %d fails: %s", __FUNCTION__, buffer.index,
            strerror(errno));
      return -errno;
    }
  }

  ALOGI("%s: start V4L2 streaming %dx%d@%ffps", __FUNCTION__, v4l2Fmt.width,
        v4l2Fmt.height, fps);
...
std::unique_ptr<V4L2Frame>
VirtioMediaCameraDeviceSession::dequeueV4l2FrameLocked(nsecs_t* shutterTs) {
  ATRACE_CALL();
  ...
  do {
    ...
    if (bytesused > mMaxV4L2BufferSize) {
      ALOGE("%s: v4l2 buffer bytes used: %u maximum %u", __FUNCTION__,
            bytesused, mMaxV4L2BufferSize);
      return ret;
    }
    ...
    if (buffer.flags & V4L2_BUF_FLAG_TIMESTAMP_MONOTONIC) {
      ...
      // even V4L2_BUF_FLAG_TSTAMP_SRC_EOF is better than capture a timestamp
      // now
      *shutterTs =
          static_cast<nsecs_t>(buffer.timestamp.tv_sec) * 1000000000LL +
          buffer.timestamp.tv_usec * 1000LL;
    } else {
      *shutterTs = curTimeNs;
    }

    // The tactic only takes effect on v4l2 buffers with flag
    // V4L2_BUF_FLAG_TIMESTAMP_MONOTONIC. Most USB cameras should have the
    // feature.
    if (curTimeNs < *shutterTs) {
      lagNs = 0;
      ALOGW(
          "%s: should not happen, the monotonic clock has issue, shutterTs is "
          "in the "
    ...
    if (lagNs > mMaxLagNs) {
      ALOGI(
          "%s: drop too old buffer, index %d, lag %" PRIu64 " ns > max %" PRIu64
          " ns. "
          "bufferTs: %" PRId64 ", curTime: %" PRId64,
          __FUNCTION__, buffer.index, lagNs, mMaxLagNs, *shutterTs, curTimeNs);
      ...
      if (retVal) {
        ALOGE("%s: unexpected VIDIOC_QBUF failed, retVal %d", __FUNCTION__,
              retVal);
        return ret;
      }
    }
  ...
  std::vector<size_t> bytesused_vec;
  std::vector<uint32_t> data_offsets;
  if (isMultiPlanar(mCaptureType)) {
    for (uint32_t i = 0; i < mV4L2PlaneCount; i++) {
      bytesused_vec.push_back(planes[i].bytesused);
      data_offsets.push_back(planes[i].data_offset);
    }
  } else {
    bytesused_vec.push_back(buffer.bytesused);
    data_offsets.push_back(0);
  }

  const V4L2BufferInfo& info = mV4L2Buffers[buffer.index];
  auto [addrs, _] = info.getAddrsAndSizes();
  return std::make_unique<V4L2Frame>(
      mV4l2StreamingFmt.width, mV4l2StreamingFmt.height,
      mV4l2StreamingFmt.fourcc, buffer.index, mV4l2Fd.get(), bytesused_vec,
      addrs, mV4L2BytesPerLine, data_offsets);
}

void VirtioMediaCameraDeviceSession::enqueueV4l2Frame(
    const std::shared_ptr<V4L2Frame>& frame) {
  ATRACE_CALL();
  ...
  if (isMultiPlanar(mCaptureType)) {
    buffer.m.planes = mV4L2Buffers[frame->mBufferIndex].planes;
    buffer.length = mV4L2PlaneCount;
  }
  if (TEMP_FAILURE_RETRY(ioctl(mV4l2Fd.get(), VIDIOC_QBUF, &buffer)) < 0) {
    ALOGE("%s: QBUF index %d fails: %s", __FUNCTION__, frame->mBufferIndex,
          strerror(errno));
    return;
  }
  ATRACE_END();
...
bool VirtioMediaCameraDeviceSession::isSupported(
    const Stream& stream,
  ...
  switch (fmt) {
    case PixelFormat::BLOB:
      if (ds != Dataspace::JFIF) {
        ALOGI("%s: BLOB format does not support dataSpace %x", __FUNCTION__,
              ds);
        return false;
      }
      break;
    ...
    case PixelFormat::Y16:
      ...
      if (!(static_cast<int32_t>(ds) &
            static_cast<int32_t>(Dataspace::DEPTH))) {
  ...
  // Assume we can convert any V4L2 format to any of supported output format for
  // now, i.e. ignoring v4l2Fmt.fourcc for now. Might need more subtle check if
  // we support more v4l format in the futrue.
...
Status VirtioMediaCameraDeviceSession::importBuffer(
...
Status VirtioMediaCameraDeviceSession::importBufferLocked(
    int32_t streamId, uint64_t bufId, buffer_handle_t buf,
    buffer_handle_t** outBufPtr) {
  return importBufferImpl(mCirculatingBuffers, sHandleImporter, streamId, bufId,
                          buf, outBufPtr);
}
...
void VirtioMediaCameraDeviceSession::closeImpl() {
  Mutex::Autolock _il(mInterfaceLock);
...
Status VirtioMediaCameraDeviceSession::selectV4L2Format(
    const StreamConfiguration& in_requestedConfiguration,
  ...
  float desiredAr =
      (mCroppingType == VERTICAL) ? kMaxAspectRatio : kMinAspectRatio;
  uint32_t maxDim = 0;
  for (const auto& stream : in_requestedConfiguration.streams) {
  ...
  // Determine Target FPS from session parameters
  double targetFps = 0.0;
  const camera_metadata_t* sessionParams = nullptr;
  if (!in_requestedConfiguration.sessionParams.metadata.empty()) {
    if (convertFromAidl(in_requestedConfiguration.sessionParams,
                        &sessionParams) &&
        sessionParams != nullptr) {
      camera_metadata_ro_entry entry;
      int ret = find_camera_metadata_ro_entry(
          sessionParams, ANDROID_CONTROL_AE_TARGET_FPS_RANGE, &entry);
      if (ret == 0 && entry.count == 2) {
        targetFps = entry.data.i32[1];
      }
    }
  }

  auto getMaxFps = [](const SupportedV4L2Format& format) {
    double maxFps = 0.0;
    for (const auto& fr : format.frameRates) {
      maxFps = std::max(maxFps, fr.getFramesPerSecond());
    }
    return maxFps;
  };

  auto selectBetterFormat =
      [&](const SupportedV4L2Format& cand,
          const SupportedV4L2Format& fmt) -> const SupportedV4L2Format& {
    // Helper lambda to calculate a "score" for a given format
    auto getScore = [&](const SupportedV4L2Format& f) {
      double fps = getMaxFps(f);

      if (targetFps <= 0.0) {
        // If targetFps is 0, we only care about the highest framerate.
        // We fill the first two priorities with 'true' so only the FPS
        // tie-breaker matters.
        return std::make_tuple(true, true, fps);
      }

      bool meetsTarget = (fps >= targetFps);
      bool isBetterQuality = (f.fourcc != V4L2_PIX_FMT_MJPEG);

      // Score Priority 1: Does it meet the target FPS? (true > false)
      // Score Priority 2: Is it a preferred format? (Non-MJPEG > MJPEG)
      // Score Priority 3: What is the absolute highest framerate?
      return std::make_tuple(meetsTarget, isBetterQuality, fps);
    };

    // The tuple's > operator natively compares Priority 1, then 2, then 3.
    return (getScore(fmt) > getScore(cand)) ? fmt : cand;
  };

  // Select the optimal V4L2 format and resolution to satisfy the requested
  // stream configuration. Policy Hierarchy:
  // 1. Resolution: Find the closest matching resolution >= requested
  // dimensions.
  //    Once the minimal viable resolution is identified, it is locked in.
  // 2. Framerate: Within that resolution, prefer formats that satisfy the
  // target FPS range.
  // 3. Format Preference: Prefer uncompressed YUV/NV12 over MJPEG to eliminate
  // decompression
  //    overhead.
  SupportedV4L2Format v4l2Fmt{.width = 0, .height = 0};
  for (const auto& fmt : mSupportedFormats) {
    ...
    if (dim < maxDim) {
      // Skip dimensions smaller than what we need.
      continue;
    }

    float aspectRatio = ASPECT_RATIO(fmt);
    if (!isAspectRatioClose(aspectRatio, desiredAr)) {
      // Skip dimensions in the aspect ratio not close enough to the
      // request.
      continue;
    }

    // Since mSupportedFormats is sorted from smallest to largest resolution,
    // the first matching size we find is our optimal resolution.
    if (v4l2Fmt.width == 0) {
      v4l2Fmt = fmt;
    } else if (v4l2Fmt.width != fmt.width || v4l2Fmt.height != fmt.height) {
      // We already found an optimal resolution match and have moved past it.
      break;
    }

    v4l2Fmt = selectBetterFormat(v4l2Fmt, fmt);
  }
  ...
  for (const auto& fmt : mSupportedFormats) {
    ...
    if ((mCroppingType == VERTICAL && aspectRatio < desiredAr) ||
        (mCroppingType == HORIZONTAL && aspectRatio > desiredAr)) {
      // Skip dimensions with unsatisfying aspect ratios.
      continue;
    }
  ...
  ALOGE(
      "%s: unable to find a resolution matching (%s at least %d, aspect ratio "
      "%f)",
      __FUNCTION__, (mCroppingType == VERTICAL) ? "width" : "height", maxDim,
      desiredAr);
  return Status::ILLEGAL_ARGUMENT;
}

int VirtioMediaCameraDeviceSession::v4l2StreamOffLocked() {
  if (!mV4l2Streaming) {
    return OK;
  }

  {
    ...
    if (mNumDequeuedV4l2Buffers != 0) {
      ALOGE("%s: there are %zu inflight V4L buffers", __FUNCTION__,
            mNumDequeuedV4l2Buffers);
      return -1;
    }
  }
  ...
  v4l2_buf_type capture_type = mCaptureType;
  if (TEMP_FAILURE_RETRY(
          ioctl(mV4l2Fd.get(), VIDIOC_STREAMOFF, &capture_type)) < 0) {
  ...
  mV4L2Buffers.clear();  // RAII handles munmap automatically
  if (TEMP_FAILURE_RETRY(ioctl(mV4l2Fd.get(), VIDIOC_REQBUFS, &req_buffers)) <
      0) {
...
int VirtioMediaCameraDeviceSession::setV4l2FpsLocked(double fps) {
  // VIDIOC_G_PARM/VIDIOC_S_PARM: set fps
  v4l2_streamparm streamparm = {.type = mCaptureType};
  // The following line checks that the driver knows about framerate get/set.
  int ret =
      TEMP_FAILURE_RETRY(ioctl(mV4l2Fd.get(), VIDIOC_G_PARM, &streamparm));
  if (ret != 0) {
    if (errno == -EINVAL) {
  ...
  if (TEMP_FAILURE_RETRY(ioctl(mV4l2Fd.get(), VIDIOC_S_PARM, &streamparm)) <
      0) {
    ALOGE("%s: failed to set framerate to %f: %s", __FUNCTION__, fps,
          strerror(errno));
    return -1;
  }
...
void VirtioMediaCameraDeviceSession::cleanupInflightFences(
...
void VirtioMediaCameraDeviceSession::cleanupBuffersLocked(int id) {
...
void VirtioMediaCameraDeviceSession::notifyShutter(int32_t frameNumber,
                                                   nsecs_t shutterTs) {
  NotifyMsg msg;
...
void VirtioMediaCameraDeviceSession::notifyError(int32_t frameNumber,
                                                 int32_t streamId,
                                                 ErrorCode ec) {
  NotifyMsg msg;
...
void VirtioMediaCameraDeviceSession::invokeProcessCaptureResultCallback(
    ...
    for (CaptureResult& result : results) {
      ...
      if (!md.metadata.empty()) {
        if (mResultMetadataQueue->write(
                reinterpret_cast<int8_t*>(md.metadata.data()),
                md.metadata.size())) {
          result.fmqResultSize = md.metadata.size();
          md.metadata.resize(0);
        } else {
          ALOGW("%s: couldn't utilize fmq, fall back to hwbinder",
                __FUNCTION__);
  ...
  if (!status.isOk()) {
    ALOGE("%s: processCaptureResult ERROR : %d:%d", __FUNCTION__,
...
int VirtioMediaCameraDeviceSession::waitForV4L2BufferReturnLocked(
    std::unique_lock<std::mutex>& lk) {
  ATRACE_CALL();
  auto timeout = std::chrono::seconds(kBufferWaitTimeoutSec);
  mLock.unlock();
  auto st = mV4L2BufferReturned.wait_for(lk, timeout);
  // Here we introduce an order where mV4l2BufferLock is acquired before mLock,
  // while the normal lock acquisition order is reversed. This is fine because
  // in most of cases we are protected by mInterfaceLock. The only thread that
  // can cause deadlock is the OutputThread, where we do need to make sure we
  // don't acquire mLock then mV4l2BufferLock
  mLock.lock();
...
bool VirtioMediaCameraDeviceSession::supportOfflineLocked(int32_t streamId) {
  const Stream& stream = mStreamMap[streamId];
  if (stream.format == PixelFormat::BLOB &&
      static_cast<int32_t>(stream.dataSpace) ==
          static_cast<int32_t>(Dataspace::JFIF)) {
    return true;
  }
...
bool VirtioMediaCameraDeviceSession::canDropRequest(
...
void VirtioMediaCameraDeviceSession::fillOfflineSessionInfo(
    const std::vector<int32_t>& offlineStreams,
  ...
  for (size_t i = 0; i < offlineReqs.size(); i++) {
    ...
    info->offlineRequests[i].pendingStreams.resize(
        offlineReqs[i]->buffers.size());
...
Status VirtioMediaCameraDeviceSession::isStreamCombinationSupported(
    const StreamConfiguration& config,
    const std::vector<SupportedV4L2Format>& supportedFormats,
    const ExternalCameraConfig& devCfg) {
  if (config.operationMode != StreamConfigurationMode::NORMAL_MODE) {
    ALOGE("%s: unsupported operation mode: %d", __FUNCTION__,
...
void VirtioMediaCameraDeviceSession::updateBufferCaches(
  ...
  for (auto& cache : cachesToRemove) {
    ...
    if (it != cbs.end()) {
      ...
      ALOGE("%s: stream %d buffer %" PRIu64 " is not cached", __FUNCTION__,
            cache.streamId, cache.bufferId);
    }
  }
}

Status VirtioMediaCameraDeviceSession::processCaptureRequestError(
    const std::shared_ptr<HalRequest>& req, std::vector<NotifyMsg>* outMsgs,
    std::vector<CaptureResult>* outResults) {
  ATRACE_CALL();
  // Return V4L2 buffer to V4L2 buffer queue
  std::shared_ptr<V4L2Frame> v4l2Frame =
      std::static_pointer_cast<V4L2Frame>(req->frameIn);
  enqueueV4l2Frame(v4l2Frame);

  if (outMsgs == nullptr) {
    notifyShutter(req->frameNumber, req->shutterTs);
    notifyError(/*frameNum*/ req->frameNumber, /*stream*/ -1,
                ErrorCode::ERROR_REQUEST);
  } else {
    NotifyMsg shutter;
    shutter.set<NotifyMsg::Tag::shutter>(ShutterMsg{
        .frameNumber = req->frameNumber, .timestamp = req->shutterTs});

    NotifyMsg error;
    error.set<NotifyMsg::Tag::error>(
        ErrorMsg{.frameNumber = req->frameNumber,
                 .errorStreamId = -1,
  ...
  for (size_t i = 0; i < req->buffers.size(); i++) {
    ...
    if (req->buffers[i].acquireFence >= 0) {
      // numFds = 0 for error
      native_handle_t* handle =
          native_handle_create(/*numFds*/ 0, /*numInts*/ 0);
...
Status VirtioMediaCameraDeviceSession::processCaptureResult(
  ...
  for (size_t i = 0; i < req->buffers.size(); i++) {
    ...
    if (req->buffers[i].fenceTimeout) {
      ...
      notifyError(req->frameNumber, req->buffers[i].streamId,
...
ssize_t VirtioMediaCameraDeviceSession::getJpegBufferSize(
  ...
  ssize_t jpegBufferSize =
      scaleFactor * (maxJpegBufferSize - kMinJpegBufferSize) +
      kMinJpegBufferSize;
...
binder_status_t VirtioMediaCameraDeviceSession::dump(int fd,
                                                     const char** /*args*/,
                                                     uint32_t /*numArgs*/) {
  ...
  if (!intfLocked) {
    dprintf(
        fd,
        "!! VirtioMediaCameraDeviceSession interface may be deadlocked !!\n");
  }
  ...
  SupportedV4L2Format streamingFmt;
  {
    ...
    if (!sessionLocked) {
      dprintf(fd,
              "!! VirtioMediaCameraDeviceSession mLock may be deadlocked !!\n");
    }
  ...
  std::unordered_set<uint32_t> inflightFrames;
  {
    ...
    if (!iffLocked) {
      dprintf(fd,
              "!! VirtioMediaCameraDeviceSession mInflightFramesLock may be "
              "deadlocked !!\n");
    }
  ...
  if (streaming) {
    ...
    dprintf(fd, "Current V4L2 format %c%c%c%c %dx%d @ %ffps\n",
            streamingFmt.fourcc & 0xFF, (streamingFmt.fourcc >> 8) & 0xFF,
            (streamingFmt.fourcc >> 16) & 0xFF,
            (streamingFmt.fourcc >> 24) & 0xFF, streamingFmt.width,
            streamingFmt.height, mV4l2StreamingFps);
...
// Start VirtioMediaCameraDeviceSession::BufferRequestThread functions
VirtioMediaCameraDeviceSession::BufferRequestThread::BufferRequestThread(
...
int VirtioMediaCameraDeviceSession::BufferRequestThread::requestBufferStart(
...

  {
    ...
    if (mRequestingBuffer) {
      ALOGE(
          "%s: BufferRequestThread does not support more than one concurrent "
          "request!",
          __FUNCTION__);
      return -1;
    }
...
int VirtioMediaCameraDeviceSession::BufferRequestThread::
    waitForBufferRequestDone(std::vector<HalStreamBuffer>* outBufReqs) {
  ...
  if (mPendingReturnBufferReqs.empty()) {
    std::chrono::milliseconds timeout =
        std::chrono::milliseconds(kReqProcTimeoutMs);
...
void VirtioMediaCameraDeviceSession::BufferRequestThread::waitForNextRequest() {
  ATRACE_CALL();
  std::unique_lock<std::mutex> lk(mLock);
  int waitTimes = 0;
...
bool VirtioMediaCameraDeviceSession::BufferRequestThread::threadLoop() {
  waitForNextRequest();
  if (exitPending()) {
    mBufferReqs.clear();
  ...
  ScopedAStatus ret =
      mCallbacks->requestStreamBuffers(mHalBufferReqs, &bufRets, &status);
  if (!ret.isOk()) {
  ...
  std::unique_lock<std::mutex> lk(mLock);
  if (status == BufferRequestStatus::OK ||
    ...
    for (size_t i = 0; i < bufRets.size(); i++) {
      ...
      switch (bufRets[i].val.getTag()) {
        ...
        case StreamBuffersVal::Tag::buffers: {
          ...
          if (hBufs.size() != 1) {
            ALOGE("%s: expect 1 buffer returned, got %zu!", __FUNCTION__,
                  hBufs.size());
            hasError = true;
            break;
          }
          ...
          // TODO: create a batch import API so we don't need to lock/unlock
          // mCbsLock repeatedly?
          lk.unlock();
          ...
          if (s != Status::OK) {
            ALOGE("%s: stream %d import buffer failed!", __FUNCTION__,
                  streamId);
...
// End VirtioMediaCameraDeviceSession::BufferRequestThread functions

// Start VirtioMediaCameraDeviceSession::OutputThread functions

VirtioMediaCameraDeviceSession::OutputThread::OutputThread(
...
Status
VirtioMediaCameraDeviceSession::OutputThread::allocateIntermediateBuffers(
    const Size& v4lSize, const Size& thumbSize,
    const std::vector<Stream>& streams, uint32_t blobBufferSize) {
  ...
  if (mYu12Frame == nullptr || mYu12Frame->mWidth != v4lSize.width ||
    ...
    mYu12Frame.reset();
    mYu12Frame =
        std::make_shared<AllocatedFrame>(v4lSize.width, v4lSize.height);
  ...
  if (mYu12ThumbFrame == nullptr ||
    ...
    mYu12ThumbFrame =
        std::make_shared<AllocatedFrame>(thumbSize.width, thumbSize.height);
...
Status VirtioMediaCameraDeviceSession::OutputThread::submitRequest(
...
void VirtioMediaCameraDeviceSession::OutputThread::flush() {
  ATRACE_CALL();
  auto parent = mParent.lock();
  if (parent == nullptr) {
    ALOGE("%s: session has been disconnected!", __FUNCTION__);
...
void VirtioMediaCameraDeviceSession::OutputThread::dump(int fd) {
...
void VirtioMediaCameraDeviceSession::OutputThread::setExifMakeModel(
...
VirtioMediaCameraDeviceSession::OutputThread::switchToOffline() {
  ATRACE_CALL();
  auto parent = mParent.lock();
  if (parent == nullptr) {
    ALOGE("%s: session has been disconnected!", __FUNCTION__);
  ...
  ALOGV("%s: returning %zu request for offline processing", __FUNCTION__,
        reqs.size());
  return reqs;
}

int VirtioMediaCameraDeviceSession::OutputThread::requestBufferStart(
...
int VirtioMediaCameraDeviceSession::OutputThread::waitForBufferRequestDone(
...
void VirtioMediaCameraDeviceSession::OutputThread::waitForNextRequest(
...
void VirtioMediaCameraDeviceSession::OutputThread::signalRequestDone() {
  std::unique_lock<std::mutex> lk(mRequestListLock);
...
int VirtioMediaCameraDeviceSession::OutputThread::cropAndScaleLocked(
  ...
  if (ret != 0) {
    ALOGE("%s: failed to compute crop rect for output size %dx%d", __FUNCTION__,
  ...
  if (it != mScaledYu12Frames.end()) {
    ...
    if (it == mIntermediateBuffers.end()) {
      ALOGE("%s: failed to find intermediate buffer size %dx%d", __FUNCTION__,
            outSz.width, outSz.height);
      return -1;
    }
    scaledYu12Buf = it->second;
  ...
  ret = libyuv::I420Scale(
      ...
      outLayout.yStride, static_cast<uint8_t*>(outLayout.cb), outLayout.cStride,
  ...
  if (ret != 0) {
    ALOGE("%s: failed to scale buffer from %dx%d to %dx%d. Ret %d",
          __FUNCTION__, inputCrop.width, inputCrop.height, outSz.width,
          outSz.height, ret);
    return ret;
  }
...
int VirtioMediaCameraDeviceSession::OutputThread::cropAndScaleThumbLocked(
    std::shared_ptr<AllocatedFrame>& in, const Size& outSz, YCbCrLayout* out) {
  Size inSz{in->mWidth, in->mHeight};
  ...
  if ((outSz.width * outSz.height) >
    ...
    ALOGE("%s: Requested thumbnail size too big (%d,%d) > (%d,%d)",
          __FUNCTION__, outSz.width, outSz.height, mYu12ThumbFrame->mWidth,
          mYu12ThumbFrame->mHeight);
    return -1;
  }
  ...
  IMapper::Rect inputCrop{
      .left = 2 * static_cast<int32_t>((inSz.width - cropSz.width) / 4),
  ...
  if ((inputCrop.top < 0) ||
    ...
    ALOGE("%s: input layout %dx%d to for output size %dx%d", __FUNCTION__,
          inSz.width, inSz.height, outSz.width, outSz.height);
    ALOGE("%s: computed input crop +%d,+%d %dx%d", __FUNCTION__, inputCrop.left,
          inputCrop.top, inputCrop.width, inputCrop.height);
    return -1;
  }
  ...
  ALOGV("%s: crop input layout %dx%d to for output size %dx%d", __FUNCTION__,
        inSz.width, inSz.height, outSz.width, outSz.height);
  ALOGV("%s: computed input crop +%d,+%d %dx%d", __FUNCTION__, inputCrop.left,
        inputCrop.top, inputCrop.width, inputCrop.height);
  ...
  ret = libyuv::I420Scale(
      static_cast<uint8_t*>(inputLayout.y), inputLayout.yStride,
      static_cast<uint8_t*>(inputLayout.cb), inputLayout.cStride,
      static_cast<uint8_t*>(inputLayout.cr), inputLayout.cStride,
      inputCrop.width, inputCrop.height, static_cast<uint8_t*>(outFullLayout.y),
      outFullLayout.yStride, static_cast<uint8_t*>(outFullLayout.cb),
      outFullLayout.cStride, static_cast<uint8_t*>(outFullLayout.cr),
      outFullLayout.cStride, outSz.width, outSz.height,
      libyuv::FilterMode::kFilterNone);
...
int VirtioMediaCameraDeviceSession::OutputThread::createJpegLocked(
  ...
  ALOGV("%s: HAL buffer sid: %d bid: %" PRIu64 " w: %u h: %u", __FUNCTION__,
        halBuf.streamId, static_cast<uint64_t>(halBuf.bufferId), halBuf.width,
        halBuf.height);
  ALOGV("%s: HAL buffer fmt: %x usage: %" PRIx64 " ptr: %p", __FUNCTION__,
        halBuf.format, static_cast<uint64_t>(halBuf.usage), halBuf.bufPtr);
  ALOGV("%s: YV12 buffer %d x %d", __FUNCTION__, mYu12Frame->mWidth,
        mYu12Frame->mHeight);
  ...
  if (maxJpegCodeSize < 0) {
    return lfail("%s: getJpegBufferSize returned %zd", __FUNCTION__,
                 maxJpegCodeSize);
  }
  ...
  if (outputThumbnail) {
    ret = encodeJpegYU12(thumbSize, yu12Thumb, thumbQuality, 0, 0,
                         &thumbCode[0], maxThumbCodeSize, thumbCodeSize);

    if (ret != 0) {
      return lfail("%s: thumbnail encodeJpegYU12 failed with %d", __FUNCTION__,
                   ret);
    }
  }
  ...
  ret = utils->generateApp1(outputThumbnail ? &thumbCode[0] : nullptr,
                            thumbCodeSize);
  ...
  void* bufPtr = sHandleImporter.lock(
      *(halBuf.bufPtr), static_cast<uint64_t>(halBuf.usage), maxJpegCodeSize);
  ...
  ret = encodeJpegYU12(jpegSize, yu12Main, jpegQuality, exifData, exifDataSize,
  ...
  void* blobDst = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(bufPtr) +
                                          maxJpegCodeSize - sizeof(CameraBlob));
  ...
  ALOGV("%s: encoded JPEG (ret:%d) with Q:%d max size: %zu", __FUNCTION__, ret,
        jpegQuality, maxJpegCodeSize);

  return 0;
}

void VirtioMediaCameraDeviceSession::OutputThread::clearIntermediateBuffers() {
  std::lock_guard<std::mutex> lk(mBufferLock);
  mYu12Frame.reset();
  mYu12ThumbFrame.reset();
...
bool VirtioMediaCameraDeviceSession::OutputThread::threadLoop() {
  std::shared_ptr<HalRequest> req;
  auto parent = mParent.lock();
  if (parent == nullptr) {
    ALOGE("%s: session has been disconnected!", __FUNCTION__);
  ...
  auto onDeviceError = [&](auto... args) {
    ALOGE(args...);
    parent->notifyError(req->frameNumber, /*stream*/ -1,
  ...
  if (req->frameIn->mFourcc != V4L2_PIX_FMT_MJPEG &&
      ...
      req->frameIn->mFourcc != V4L2_PIX_FMT_YUV420M &&
      req->frameIn->mFourcc != V4L2_PIX_FMT_NV12 &&
      req->frameIn->mFourcc != V4L2_PIX_FMT_NV12M) {
    return onDeviceError("%s: do not support V4L2 format %c%c%c%c",
                         __FUNCTION__, req->frameIn->mFourcc & 0xFF,
  ...
  if (testPatternMode.count == 1) {
    if (mCameraMuted !=
        (testPatternMode.data.u8[0] != ANDROID_SENSOR_TEST_PATTERN_MODE_OFF)) {
      mCameraMuted = !mCameraMuted;
      // Get solid color for test pattern, if any was set
      if (testPatternMode.data.u8[0] ==
        ...
        if (entry.count == 4) {
          ...
          if (memcmp(entry.data.i32, mTestPatternData,
                     sizeof(mTestPatternData)) != 0) {
            memcpy(mTestPatternData, entry.data.i32, sizeof(mTestPatternData));
            // Fill the mute frame with the solid color, use only 8 MSB of RGGB
            // as RGB
  ...
  // TODO: in some special case maybe we can decode jpg directly to gralloc
  // output?
  res = 0;
  if (mCameraMuted && req->frameIn->mFourcc != V4L2_PIX_FMT_Z16) {
    ATRACE_BEGIN("MutePatternToI420");
    res = libyuv::ConvertToI420(
        ...
        static_cast<uint8_t*>(mYu12FrameLayout.cr), mYu12FrameLayout.cStride, 0,
        0, mYu12Frame->mWidth, mYu12Frame->mHeight, mYu12Frame->mWidth,
    ...
    ATRACE_END();
             ...
             req->frameIn->mFourcc == V4L2_PIX_FMT_YUV420M) {
    ATRACE_BEGIN("YU12copy");
    std::shared_ptr<V4L2Frame> v4l2Frame =
        std::static_pointer_cast<V4L2Frame>(req->frameIn);
    uint8_t *y = nullptr, *cb = nullptr, *cr = nullptr;
    size_t y_size, cb_size, cr_size;

    if (v4l2Frame->mFourcc == V4L2_PIX_FMT_YUV420M) {
      if (v4l2Frame->getPlaneData(0, &y, &y_size) != 0 ||
          v4l2Frame->getPlaneData(1, &cb, &cb_size) != 0 ||
          v4l2Frame->getPlaneData(2, &cr, &cr_size) != 0) {
        return onDeviceError("%s: YUV420M must have 3 planes", __FUNCTION__);
      }
      // Explicit bounds check to prevent SIGSEGV on truncated V4L2 payloads
      size_t min_y_size = v4l2Frame->mBytesPerLine[0] * mYu12Frame->mHeight;
      size_t min_cb_size =
          v4l2Frame->mBytesPerLine[1] * (mYu12Frame->mHeight / 2);
      size_t min_cr_size =
          v4l2Frame->mBytesPerLine[2] * (mYu12Frame->mHeight / 2);
      if (y_size < min_y_size || cb_size < min_cb_size ||
          cr_size < min_cr_size) {
        return onDeviceError(
            "%s: Dequeued YUV420M planes truncated (Y:%zu<%zu, Cb:%zu<%zu, "
            "Cr:%zu<%zu)",
            __FUNCTION__, y_size, min_y_size, cb_size, min_cb_size, cr_size,
            min_cr_size);
      }
    } else {
      v4l2Frame->getPlaneData(0, &y, &y_size);
      uint32_t stride = v4l2Frame->mBytesPerLine[0];
      // Explicit bounds check to prevent SIGSEGV on truncated V4L2 payloads
      size_t min_total_size = stride * mYu12Frame->mHeight * 3 / 2;
      if (y_size < min_total_size) {
        return onDeviceError("%s: Dequeued YUV420 buffer truncated (%zu < %zu)",
                             __FUNCTION__, y_size, min_total_size);
      }
      cb = y + (stride * mYu12Frame->mHeight);
      cr = cb + ((stride / 2) * (mYu12Frame->mHeight / 2));
    }

    res = libyuv::I420Copy(
        y, v4l2Frame->mBytesPerLine[0], cb,
        (v4l2Frame->mFourcc == V4L2_PIX_FMT_YUV420M)
            ? v4l2Frame->mBytesPerLine[1]
            : v4l2Frame->mBytesPerLine[0] / 2,
        cr,
        (v4l2Frame->mFourcc == V4L2_PIX_FMT_YUV420M)
            ? v4l2Frame->mBytesPerLine[2]
            : v4l2Frame->mBytesPerLine[0] / 2,
        static_cast<uint8_t*>(mYu12FrameLayout.y), mYu12FrameLayout.yStride,
        static_cast<uint8_t*>(mYu12FrameLayout.cb), mYu12FrameLayout.cStride,
    ...
    ATRACE_END();
             ...
             req->frameIn->mFourcc == V4L2_PIX_FMT_NV12M) {
    ATRACE_BEGIN("NV12copy");
    std::shared_ptr<V4L2Frame> v4l2Frame =
        std::static_pointer_cast<V4L2Frame>(req->frameIn);
    uint8_t *y = nullptr, *uv = nullptr;
    size_t y_size, uv_size;

    if (v4l2Frame->mFourcc == V4L2_PIX_FMT_NV12M) {
      if (v4l2Frame->getPlaneData(0, &y, &y_size) != 0 ||
          v4l2Frame->getPlaneData(1, &uv, &uv_size) != 0) {
        return onDeviceError("%s: NV12M must have 2 planes", __FUNCTION__);
      }
      // Explicit bounds check to prevent SIGSEGV on truncated V4L2 payloads
      size_t min_y_size = v4l2Frame->mBytesPerLine[0] * mYu12Frame->mHeight;
      size_t min_uv_size =
          v4l2Frame->mBytesPerLine[1] * (mYu12Frame->mHeight / 2);
      if (y_size < min_y_size || uv_size < min_uv_size) {
        return onDeviceError(
            "%s: Dequeued NV12M planes truncated (Y:%zu<%zu, UV:%zu<%zu)",
            __FUNCTION__, y_size, min_y_size, uv_size, min_uv_size);
      }
    } else {
      v4l2Frame->getPlaneData(0, &y, &y_size);
      uint32_t stride = v4l2Frame->mBytesPerLine[0];
      ...
      size_t min_total_size = stride * mYu12Frame->mHeight * 3 / 2;
      if (y_size < min_total_size) {
        return onDeviceError("%s: Dequeued NV12 buffer truncated (%zu < %zu)",
                             __FUNCTION__, y_size, min_total_size);
      }
      uv = y + (stride * mYu12Frame->mHeight);
    }

    res = libyuv::NV12ToI420(
        y, v4l2Frame->mBytesPerLine[0], uv,
        (v4l2Frame->mFourcc == V4L2_PIX_FMT_NV12M)
            ? v4l2Frame->mBytesPerLine[1]
            : v4l2Frame->mBytesPerLine[0],
        static_cast<uint8_t*>(mYu12FrameLayout.y), mYu12FrameLayout.yStride,
        static_cast<uint8_t*>(mYu12FrameLayout.cb), mYu12FrameLayout.cStride,
        static_cast<uint8_t*>(mYu12FrameLayout.cr), mYu12FrameLayout.cStride,
        mYu12Frame->mWidth, mYu12Frame->mHeight);
  ...
  if (res != 0) {
    ...
    if (st != Status::OK) {
      return onDeviceError("%s: failed to process capture request error!",
                           __FUNCTION__);
    }
    signalRequestDone();
    return true;
  ...
  for (auto& halBuf : req->buffers) {
    if (halBuf.bufPtr == nullptr) {
      // This can happen if mBufferRequestThread is closed before bufPtr is
      // filled, typically when the session is closing. Treat it as a import
      // failure and move on.
      ALOGW("%s: Could not import buffer for stream %d", __FUNCTION__,
            halBuf.streamId);
    ...
    switch (halBuf.format) {
      ...
      case PixelFormat::YV12: {
        ...
        ALOGV("%s: outLayout y %p cb %p cr %p y_str %zu c_str %zu c_step %zu",
              __FUNCTION__, result.y, result.cb, result.cr, result.ystride,
              result.cstride, result.chroma_step);
        if (result.ystride > UINT32_MAX || result.cstride > UINT32_MAX ||
            result.chroma_step > UINT32_MAX) {
          return onDeviceError("%s: lockYCbCr failed. Unexpected values!",
                               __FUNCTION__);
        }
        YCbCrLayout outLayout = {
            .y = result.y,
        ...
        ALOGV("%s: converting to format %c%c%c%c", __FUNCTION__,
              outputFourcc & 0xFF, (outputFourcc >> 8) & 0xFF,
              (outputFourcc >> 16) & 0xFF, (outputFourcc >> 24) & 0xFF);

        YCbCrLayout cropAndScaled;
        ATRACE_BEGIN("cropAndScaleLocked");
        int ret = cropAndScaleLocked(
            mYu12Frame, Size{halBuf.width, halBuf.height}, &cropAndScaled);
        ATRACE_END();
        if (ret != 0) {
          lk.unlock();
      ...
      default:
        lk.unlock();
        return onDeviceError("%s: unknown output format %x", __FUNCTION__,
                             halBuf.format);
    }
  }  // for each buffer
...
// End VirtioMediaCameraDeviceSession::OutputThread functions
