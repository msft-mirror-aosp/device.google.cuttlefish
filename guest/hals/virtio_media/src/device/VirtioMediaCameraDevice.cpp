...
#define LOG_TAG "VirtioMediaCamDev"
// #define LOG_NDEBUG 0
...
// * V4L2_PIX_FMT_MJPEG: Standard highly-compressed webcam format.
// * V4L2_PIX_FMT_Z16: Standard 16-bit depth format.
// * V4L2_PIX_FMT_YUV420: Standard single-planar YU12.
// * V4L2_PIX_FMT_YUV420M: Multi-planar YU12 (e.g. used by virtio-media).
// * V4L2_PIX_FMT_NV12: Single-planar NV12.
// * V4L2_PIX_FMT_NV12M: Multi-planar NV12.
const std::array<uint32_t, /*size*/ 6> kSupportedFourCCs{
    {V4L2_PIX_FMT_MJPEG, V4L2_PIX_FMT_Z16, V4L2_PIX_FMT_YUV420,
     V4L2_PIX_FMT_YUV420M, V4L2_PIX_FMT_NV12,
     V4L2_PIX_FMT_NV12M}};  // double braces required in C++11
...
ndk::ScopedAStatus VirtioMediaCameraDevice::getCameraCharacteristics(
...
ndk::ScopedAStatus VirtioMediaCameraDevice::getPhysicalCameraCharacteristics(
    const std::string&, CameraMetadata*) {
  ALOGE("%s: Physical camera functions are not supported for external cameras.",
        __FUNCTION__);
  return fromStatus(Status::ILLEGAL_ARGUMENT);
}

ndk::ScopedAStatus VirtioMediaCameraDevice::getResourceCost(
...
ndk::ScopedAStatus VirtioMediaCameraDevice::isStreamCombinationSupported(
    const StreamConfiguration& in_streams, bool* _aidl_return) {
  if (isInitFailed()) {
    ALOGE("%s: camera %s. camera init failed!", __FUNCTION__,
          mCameraId.c_str());
    return fromStatus(Status::INTERNAL_ERROR);
  }
...
ndk::ScopedAStatus VirtioMediaCameraDevice::open(
  ...
  Mutex::Autolock _l(mLock);
  if (isInitFailedLocked()) {
    ALOGE("%s: cannot open camera %s. camera init failed!", __FUNCTION__,
          mCameraId.c_str());
    return fromStatus(Status::INTERNAL_ERROR);
  }
  ...
  while (fd.get() < 0 && numAttempt < MAX_RETRY) {
    // Previous retry attempts failed. Retry opening the device at most
    // MAX_RETRY times
  ...
  if (fd.get() < 0) {
    ALOGE("%s: v4l2 device open %s failed: %s", __FUNCTION__,
          mDevicePath.c_str(), strerror(errno));
    return fromStatus(Status::INTERNAL_ERROR);
  }
...
ndk::ScopedAStatus VirtioMediaCameraDevice::openInjectionSession(
...
ndk::ScopedAStatus VirtioMediaCameraDevice::setTorchMode(bool) {
  return fromStatus(Status::OPERATION_NOT_SUPPORTED);
}

ndk::ScopedAStatus VirtioMediaCameraDevice::turnOnTorchWithStrengthLevel(
    int32_t) {
  return fromStatus(Status::OPERATION_NOT_SUPPORTED);
}

ndk::ScopedAStatus VirtioMediaCameraDevice::getTorchStrengthLevel(int32_t*) {
  return fromStatus(Status::OPERATION_NOT_SUPPORTED);
}
...
bool VirtioMediaCameraDevice::isInitFailed() {
  Mutex::Autolock _l(mLock);
  return isInitFailedLocked();
}

bool VirtioMediaCameraDevice::isInitFailedLocked() {
  if (!mInitialized) {
    status_t ret = initCameraCharacteristics();
    if (ret != OK) {
      ALOGE("%s: init camera characteristics failed: errorno %d", __FUNCTION__,
            ret);
      mInitFailed = true;
    }
    mInitialized = true;
...
void VirtioMediaCameraDevice::initSupportedFormatsLocked(int fd) {
...
status_t VirtioMediaCameraDevice::initCameraCharacteristics() {
  ...
  mCaptureType = getCaptureType(fd.get());

  status_t ret;
  ret = initDefaultCharsKeys(&mCameraCharacteristics);
  if (ret != OK) {
    ALOGE("%s: init default characteristics key failed: errorno %d",
          __FUNCTION__, ret);
  ...
  if (ret != OK) {
    ALOGE("%s: init camera control characteristics key failed: errorno %d",
          __FUNCTION__, ret);
    mCameraCharacteristics.clear();
    return ret;
  }
  ...
  if (ret != OK) {
    ALOGE("%s: init output characteristics key failed: errorno %d",
          __FUNCTION__, ret);
    mCameraCharacteristics.clear();
    return ret;
  }
  ...
  if (ret != OK) {
    ALOGE("%s: init available capabilities key failed: errorno %d",
          __FUNCTION__, ret);
    mCameraCharacteristics.clear();
    return ret;
  }
...
status_t VirtioMediaCameraDevice::initAvailableCapabilities(
...
status_t VirtioMediaCameraDevice::initDefaultCharsKeys(
  ...
  // android.info
  const uint8_t bufMgrVer =
      ANDROID_INFO_SUPPORTED_BUFFER_MANAGEMENT_VERSION_HIDL_DEVICE_3_5;
  UPDATE(ANDROID_INFO_SUPPORTED_BUFFER_MANAGEMENT_VERSION, &bufMgrVer, 1);

  // android.jpeg
  const int32_t jpegAvailableThumbnailSizes[] = {
      0, 0, 176, 144, 240, 144, 256, 144, 240, 160, 256, 154, 240, 180};
  ...
  // Orientation is a bit odd for external camera, but consider it as the
  // orientation between the external camera sensor (which is usually landscape)
  // and the device's natural display orientation. For devices with natural
  // landscape display (ex: tablet/TV), the orientation should be 0. For devices
  // with natural portrait display (phone), the orientation should be 270.
  ...
  const uint8_t sensorReadoutTimestamp =
   ...
   * android.sensor.info.preCorrectionActiveArraySize -> not available/not
   * needed android.sensor.blackLevelPattern           -> not available/not
   * needed
   */
...
status_t VirtioMediaCameraDevice::initOutputCharsKeys(
    int fd, ::android::hardware::camera::common::V1_0::helper::CameraMetadata*
                metadata) {
  ...
  std::vector<uint32_t> colorFourccs;
  std::vector<uint32_t> depthFourccs;
  ...
  for (const auto& supportedFormat : mSupportedFormats) {
    switch (supportedFormat.fourcc) {
      case V4L2_PIX_FMT_Z16:
        if (std::find(depthFourccs.begin(), depthFourccs.end(),
                      supportedFormat.fourcc) == depthFourccs.end()) {
          depthFourccs.push_back(supportedFormat.fourcc);
        }
        break;
      case V4L2_PIX_FMT_MJPEG:
      case V4L2_PIX_FMT_YUV420:
      ...
      case V4L2_PIX_FMT_NV12:
      case V4L2_PIX_FMT_NV12M:
        if (std::find(colorFourccs.begin(), colorFourccs.end(),
                      supportedFormat.fourcc) == colorFourccs.end()) {
          colorFourccs.push_back(supportedFormat.fourcc);
        }
        break;
      default:
  ...
  if (!depthFourccs.empty()) {
    status_t ret = initOutputCharsKeysByFormat(
        metadata, depthFourccs, halDepthFormats,
  ...
  if (!colorFourccs.empty()) {
    status_t ret = initOutputCharsKeysByFormat(
        metadata, colorFourccs, halFormats,
  ...
  if (ret != OK) {
    ALOGE("%s: Unable to update fps metadata: %s", __FUNCTION__,
          statusToString(ret).c_str());
    return ret;
  }
...
status_t VirtioMediaCameraDevice::initOutputCharsKeysByFormat(
    ::android::hardware::camera::common::V1_0::helper::CameraMetadata* metadata,
    const std::vector<uint32_t>& fourccs,
    const std::array<int, SIZE>& halFormats, int streamConfigTag,
  ...
  // The camera might support the same resolution in multiple formats (e.g.
  // MJPEG and YUV). The Android Camera API expects a single minimum frame
  // duration for a given size. We take the union of sizes over all supported
  // fourccs to find the absolute minimum frame duration the hardware can
  // achieve for each size. Key: {width, height}, Value: minFrameDuration
  std::map<std::tuple<int32_t, int32_t>, int64_t> sizeToMinFrameDuration;

  for (const auto& supportedFormat : mSupportedFormats) {
    if (std::find(fourccs.begin(), fourccs.end(), supportedFormat.fourcc) ==
        fourccs.end()) {
    ...
    for (const auto& fr : supportedFormat.frameRates) {
      ...
      int64_t frameDuration =
          1000000000LL * fr.durationNumerator / fr.durationDenominator;
      if (frameDuration < minFrameDuration) {
        minFrameDuration = frameDuration;
      }
    }

    auto size = std::make_tuple(supportedFormat.width, supportedFormat.height);
    if (!sizeToMinFrameDuration.contains(size) ||
        minFrameDuration < sizeToMinFrameDuration[size]) {
      sizeToMinFrameDuration[size] = minFrameDuration;
    }
  }
  ...
  for (const auto& format : halFormats) {
    for (const auto& [size, minDuration] : sizeToMinFrameDuration) {
      auto [width, height] = size;

      // Stream Configuration
      streamConfigurations.push_back(format);
      streamConfigurations.push_back(width);
      streamConfigurations.push_back(height);
      streamConfigurations.push_back(streamConfigTag);

      // Min Frame Duration
      minFrameDurations.push_back(format);
      minFrameDurations.push_back(width);
      minFrameDurations.push_back(height);
      minFrameDurations.push_back(minDuration);
      ...
      stallDurations.push_back(width);
      stallDurations.push_back(height);
      stallDurations.push_back(stall_duration);
    }
  }

  UPDATE(streamConfigurationKey, streamConfigurations.data(),
         streamConfigurations.size());

  UPDATE(minFrameDurationKey, minFrameDurations.data(),
...
status_t VirtioMediaCameraDevice::calculateMinFps(
    ::android::hardware::camera::common::V1_0::helper::CameraMetadata*
        metadata) {
...
void VirtioMediaCameraDevice::getFrameRateList(int fd, double fpsUpperBound,
    ...
    if (frameInterval.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
      if (frameInterval.discrete.numerator != 0) {
        ...
        ALOGV("index:%d, format:%c%c%c%c, w %d, h %d, framerate %f",
              frameInterval.index, frameInterval.pixel_format & 0xFF,
  ...
  if (format->frameRates.empty()) {
    ALOGE(
        "%s: failed to get supported frame rates for format:%c%c%c%c w %d h %d",
        __FUNCTION__, frameInterval.pixel_format & 0xFF,
        (frameInterval.pixel_format >> 8) & 0xFF,
        (frameInterval.pixel_format >> 16) & 0xFF,
        (frameInterval.pixel_format >> 24) & 0xFF, frameInterval.width,
        frameInterval.height);
  }
}

void VirtioMediaCameraDevice::updateFpsBounds(
...
VirtioMediaCameraDevice::getCandidateSupportedFormatsLocked(
    int fd, CroppingType cropType,
    const std::vector<ExternalCameraConfig::FpsLimitation>& fpsLimits,
    const std::vector<ExternalCameraConfig::FpsLimitation>& depthFpsLimits,
    const Size& minStreamSize, bool depthEnabled) {
  std::vector<SupportedV4L2Format> outFmts;
  struct v4l2_fmtdesc fmtdesc{.index = 0, .type = getCaptureType(fd)};
  int ret = 0;
  while (ret == 0) {
    ...
    for (;
         TEMP_FAILURE_RETRY(ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frameSize)) == 0;
         ++frameSize.index) {
  ...
  const auto& maxSize = outFmts.back();
  ...
  for (const auto& fmt : outFmts) {
    ...
    if (isAspectRatioClose(ar, maxSizeAr)) {
      ...
      ALOGV("%s: size (%d,%d) is removed due to unable to crop %s from (%d,%d)",
            ...
            cropType == VERTICAL ? "vertically" : "horizontally", maxSize.width,
            maxSize.height);
    }
  }
  outFmts = out;
