...
using ::aidl::android::hardware::camera::provider::
...
class ExternalCameraProvider : public BnCameraProvider {
 public:
  ...
  ndk::ScopedAStatus getVendorTags(
      std::vector<VendorTagSection>* _aidl_return) override;
 ...
 private:
  ...
  // This thread calls back into ExternalCameraProvider when an actionable
  // change is detected.
  class HotplugThread : public SimpleThread {
   ...
   private:
    // Returns true if thread initialization succeeded, and false if thread
  ...
  std::unordered_map<std::string, CameraDeviceStatus>
      mCameraStatusMap;  // camera id -> status
