...
// #define LOG_NDEBUG 0
#define LOG_TAG "C2CuttlefishHevcDec"
...
constexpr char COMPONENT_NAME[] = "c2.cuttlefish.hevc.decoder";
...
using ivdext_create_op_t = ihevcd_cxa_create_op_t;
using ivdext_delete_ip_t = ihevcd_cxa_delete_ip_t;
using ivdext_delete_op_t = ihevcd_cxa_delete_op_t;
using ivdext_ctl_set_num_cores_ip_t = ihevcd_cxa_ctl_set_num_cores_ip_t;
using ivdext_ctl_set_num_cores_op_t = ihevcd_cxa_ctl_set_num_cores_op_t;
using ivdext_ctl_get_vui_params_ip_t = ihevcd_cxa_ctl_get_vui_params_ip_t;
using ivdext_ctl_get_vui_params_op_t = ihevcd_cxa_ctl_get_vui_params_op_t;

constexpr auto ivdec_api_function = ihevcd_cxa_api_function;

constexpr size_t ALIGN128(size_t x) { return ((x + 127) >> 7) << 7; }

constexpr size_t MAX_NUM_CORES = 4;
constexpr IVD_CONTROL_API_COMMAND_TYPE_T IVDEXT_CMD_CTL_SET_NUM_CORES =
    (IVD_CONTROL_API_COMMAND_TYPE_T)IHEVCD_CXA_CMD_CTL_SET_NUM_CORES;

class C2CuttlefishHevcDec::IntfImpl : public SimpleInterface<void>::BaseParams {
 public:
  explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper>& helper)
    ...
    std::vector<uint32_t> profiles = {C2Config::PROFILE_HEVC_MAIN,
                                      C2Config::PROFILE_HEVC_MAIN_STILL};
    if (android::media::swcodec::flags::hevc_400()) {
      profiles.push_back(C2Config::PROFILE_HEVC_MONO);
    }
    ...
    addParameter(
        DefineParam(mMaxSize, C2_PARAMKEY_MAX_PICTURE_SIZE)
            ...
            .withFields({
                C2F(mMaxSize, width).inRange(2, 4096, 2),
                C2F(mMaxSize, height).inRange(2, 4096, 2),
            })
...
C2CuttlefishHevcDec::C2CuttlefishHevcDec(
...
C2CuttlefishHevcDec::C2CuttlefishHevcDec(
    ...
    : C2CuttlefishHevcDec(name, id, std::make_shared<IntfImpl>(helper)) {}

C2CuttlefishHevcDec::~C2CuttlefishHevcDec() { onRelease(); }

c2_status_t C2CuttlefishHevcDec::onInit() {
  status_t err = initDecoder();
  return err == OK ? C2_OK : C2_CORRUPTED;
}

c2_status_t C2CuttlefishHevcDec::onStop() {
  if (OK != resetDecoder()) {
    return C2_CORRUPTED;
  }
  resetPlugin();
...
void C2CuttlefishHevcDec::onReset() { (void)onStop(); }

void C2CuttlefishHevcDec::onRelease() {
  (void)deleteDecoder();
  if (mOutBufferFlush) {
...
c2_status_t C2CuttlefishHevcDec::onFlush_sm() {
  if (OK != setFlushMode()) {
    return C2_CORRUPTED;
  }
  ...
  uint64_t size = (uint64_t)displayStride * displayHeight * 3 / 2;
  if (size > INT32_MAX) {
    ALOGE("Buffer size overflow: %llu", (unsigned long long)size);
    return C2_NO_MEMORY;
  }
  WORD32 bufferSize = (WORD32)size;
  mOutBufferFlush = (uint8_t*)ivd_aligned_malloc(nullptr, 128, bufferSize);
  if (!mOutBufferFlush) {
    ALOGE("could not allocate tmp output buffer (for flush) of size %d ",
          bufferSize);
    return C2_NO_MEMORY;
  }
...
status_t C2CuttlefishHevcDec::createDecoder() {
  ivdext_create_ip_t s_create_ip = {};
  ivdext_create_op_t s_create_op = {};

  s_create_ip.s_ivd_create_ip_t.u4_size = sizeof(ivdext_create_ip_t);
  s_create_ip.u4_keep_threads_active = KEEP_THREADS_ACTIVE;
  s_create_ip.u4_enable_yuv_formats = 0;
  if (android::media::swcodec::flags::hevc_400()) {
    s_create_ip.u4_enable_yuv_formats |= (1 << CHROMA_FMT_IDC_MONOCHROME);
  }
...
status_t C2CuttlefishHevcDec::setNumCores() {
  ...
  IV_API_CALL_STATUS_T status =
      ivdec_api_function(mDecHandle, &s_set_num_cores_ip, &s_set_num_cores_op);
...
status_t C2CuttlefishHevcDec::setParams(size_t stride,
...
status_t C2CuttlefishHevcDec::getVersion() {
  ...
  IV_API_CALL_STATUS_T status = ivdec_api_function(
      mDecHandle, &s_get_versioninfo_ip, &s_get_versioninfo_op);
  if (status != IV_SUCCESS) {
    ALOGD("error in %s: 0x%x", __func__, s_get_versioninfo_op.u4_error_code);
  } else {
...
status_t C2CuttlefishHevcDec::initDecoder() {
  if (OK != createDecoder()) {
    return UNKNOWN_ERROR;
  }
  mNumCores = std::min(getCpuCoreCount(), MAX_NUM_CORES);
  mStride = ALIGN128(mWidth);
  mSignalledError = false;
  resetPlugin();
  (void)setNumCores();
  if (OK != setParams(mStride, IVD_DECODE_FRAME)) {
    return UNKNOWN_ERROR;
  }
  (void)getVersion();
...
bool C2CuttlefishHevcDec::setDecodeArgs(ivd_video_decode_ip_t* ps_decode_ip,
                                        ivd_video_decode_op_t* ps_decode_op,
                                        C2ReadView* inBuffer,
  ...
  if (inBuffer) {
    ...
    ps_decode_ip->pv_stream_buffer =
        const_cast<uint8_t*>(inBuffer->data() + inOffset);
  ...
  if (outBuffer) {
    if (outBuffer->height() < displayHeight) {
      ALOGE("Output buffer too small: provided (%dx%d) required (%ux%u)",
            outBuffer->width(), outBuffer->height(), displayStride,
            displayHeight);
      return false;
    }
    if (isPlanarYUV420(*outBuffer)) {
...
bool C2CuttlefishHevcDec::getVuiParams() {
  ...
  IV_API_CALL_STATUS_T status = ivdec_api_function(
      mDecHandle, &s_get_vui_params_ip, &s_get_vui_params_op);
  if (status != IV_SUCCESS) {
...
status_t C2CuttlefishHevcDec::setFlushMode() {
  ...
  IV_API_CALL_STATUS_T status =
      ivdec_api_function(mDecHandle, &s_set_flush_ip, &s_set_flush_op);
  if (status != IV_SUCCESS) {
...
status_t C2CuttlefishHevcDec::resetDecoder() {
...
void C2CuttlefishHevcDec::resetPlugin() {
...
status_t C2CuttlefishHevcDec::deleteDecoder() {
  if (mDecHandle) {
...
void C2CuttlefishHevcDec::finishWork(uint64_t index,
  ...
  if (work && c2_cntr64_t(index) == work->input.ordinal.frameIndex) {
    ...
    // TODO: Check if cloneAndSend can be avoided by tracking number of frames
    // remaining
    if (eos) {
      if (buffer) {
        mOutIndex = index;
...
c2_status_t C2CuttlefishHevcDec::ensureDecoderState(
  ...
  if (!mOutBlock) {
    ...
    ALOGV("provided (%dx%d) required (%dx%d)", mOutBlock->width(),
          mOutBlock->height(), ALIGN128(mWidth), mHeight);
  }
...
void C2CuttlefishHevcDec::process(const std::unique_ptr<C2Work>& work,
                                  const std::shared_ptr<C2BlockPool>& pool) {
  ...
  if (!work->input.buffers.empty() &&
      !work->input.buffers[0]->data().linearBlocks().empty()) {
  ...
  while (inPos < inSize) {
    ...
    ALOGV("decodeTime=%6" PRId64 " delay=%6" PRId64 " numBytes=%6d", decodeTime,
          delay, ps_decode_op->u4_num_bytes_consumed);
    if (IVD_MEM_ALLOC_FAILED ==
    ...
    if (0 < ps_decode_op->u4_pic_wd && 0 < ps_decode_op->u4_pic_ht) {
      if (ps_decode_op->u4_pic_wd != mWidth ||
          ps_decode_op->u4_pic_ht != mHeight) {
        uint32_t newWidth = ps_decode_op->u4_pic_wd;
        uint32_t newHeight = ps_decode_op->u4_pic_ht;
        if (ps_decode_op->u4_output_present != 0u) {
          ALOGE("Output present during resolution change");
          mSignalledError = true;
          work->workletsProcessed = 1u;
          work->result = C2_CORRUPTED;
          return;
        ...
        C2StreamPictureSizeInfo::output size(0u, newWidth, newHeight);
        std::vector<std::unique_ptr<C2SettingResult>> failures;
        c2_status_t err = mIntf->config({&size}, C2_MAY_BLOCK, &failures);
        if (err == OK) {
          if (mHeaderDecoded == false) {
            mHeaderDecoded = true;
            mStride = ALIGN128(newWidth);
          ...
          ALOGE("Cannot set width and height: %u x %u", newWidth, newHeight);
          mSignalledError = true;
          work->workletsProcessed = 1u;
          work->result = C2_CORRUPTED;
          return;
    ...
    if (hasPicture && (inSize - inPos)) {
      ALOGD(
          "decoded frame in current access nal, ignoring further trailing "
          "bytes %d",
...
c2_status_t C2CuttlefishHevcDec::drainInternal(
...
c2_status_t C2CuttlefishHevcDec::drain(
    uint32_t drainMode, const std::shared_ptr<C2BlockPool>& pool) {
  return drainInternal(drainMode, pool, nullptr);
}

class C2CuttlefishHevcDecFactory : public C2ComponentFactory {
 public:
  explicit C2CuttlefishHevcDecFactory(
      const std::shared_ptr<C2ReflectorHelper>& helper)
      : mHelper(helper) {}
  ...
  virtual c2_status_t createComponent(
    ...
    *component = std::shared_ptr<C2Component>(
        new C2CuttlefishHevcDec(
            COMPONENT_NAME, id,
            std::make_shared<C2CuttlefishHevcDec::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  }
  ...
  virtual c2_status_t createInterface(
    ...
    *interface = std::shared_ptr<C2ComponentInterface>(
        new SimpleInterface<C2CuttlefishHevcDec::IntfImpl>(
            COMPONENT_NAME, id,
            std::make_shared<C2CuttlefishHevcDec::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  ...
  virtual ~C2CuttlefishHevcDecFactory() override = default;
...
std::shared_ptr<C2ComponentFactory> CreateCuttlefishHevcFactory(
    const std::shared_ptr<C2ReflectorHelper>& helper) {
  return std::make_shared<C2CuttlefishHevcDecFactory>(helper);
}

void RegisterCuttlefishHevcDec(
    CuttlefishComponentStore* store,
    const std::shared_ptr<C2ReflectorHelper>& helper) {
  auto factory = CreateCuttlefishHevcFactory(helper);
  auto traits = std::make_shared<C2Component::Traits>();
  traits->name = "c2.cuttlefish.hevc.decoder";
  traits->domain = C2Component::DOMAIN_VIDEO;
  traits->kind = C2Component::KIND_DECODER;
  traits->rank = 0x100;
  traits->mediaType = MEDIA_MIMETYPE_VIDEO_HEVC;
  traits->owner = "cuttlefish";
  traits->aliases = {};
  store->registerCodec(traits, factory);
}
