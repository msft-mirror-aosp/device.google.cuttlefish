...
const std::unordered_map<EdpMonitorName, EdidData> edpMonitorNameToData = {
...
const std::unordered_map<DpMonitorName, EdidData> dpMonitorNameToData = {
...
const std::unordered_map<HdmiMonitorName, EdidData> hdmiMonitorNameToData = {
...
std::string getEdidHexForMonitor(const MonitorName& monitorName) {
  switch (monitorName.type) {
    case MonitorName::Type::EDP:
      return edpMonitorNameToData.at(monitorName.edp).hex;
    case MonitorName::Type::DP:
      return dpMonitorNameToData.at(monitorName.dp).hex;
    case MonitorName::Type::HDMI:
      return hdmiMonitorNameToData.at(monitorName.hdmi).hex;
    default:
      return "";
  }
}
...
std::string getEdidPreferredResForMonitor(const MonitorName& monitorName) {
  switch (monitorName.type) {
    case MonitorName::Type::EDP:
      return edpMonitorNameToData.at(monitorName.edp).preferred_res;
    case MonitorName::Type::DP:
      return dpMonitorNameToData.at(monitorName.dp).preferred_res;
    case MonitorName::Type::HDMI:
      return hdmiMonitorNameToData.at(monitorName.hdmi).preferred_res;
    default:
      return "";
  }
}
...
int getEdidPreferredRrForMonitor(const MonitorName& monitorName) {
  switch (monitorName.type) {
    case MonitorName::Type::EDP:
      return edpMonitorNameToData.at(monitorName.edp).preferred_rr;
    case MonitorName::Type::DP:
      return dpMonitorNameToData.at(monitorName.dp).preferred_rr;
    case MonitorName::Type::HDMI:
      return hdmiMonitorNameToData.at(monitorName.hdmi).preferred_rr;
    default:
      return 0;
  }
}
...
#define X(monitor)                                       \
  if (android::base::EqualsIgnoreCase(name, #monitor)) { \
    return MonitorName(EdpMonitorName::monitor);         \
  }
  EDP_MONITOR_LIST(X)
#undef X
...
#define X(monitor)                                       \
  if (android::base::EqualsIgnoreCase(name, #monitor)) { \
    return MonitorName(DpMonitorName::monitor);          \
  }
  DP_MONITOR_LIST(X)
#undef X
...
#define X(monitor)                                       \
  if (android::base::EqualsIgnoreCase(name, #monitor)) { \
    return MonitorName(HdmiMonitorName::monitor);        \
  }
  HDMI_MONITOR_LIST(X)
#undef X
